...
class User_managements extends CI_Controller {
    ...
    public function index() {
        if ($this->session->userdata('is_login')) {
            $data['title'] = "User List | Adney's Shop";
    ...
    function delete($id_user) {
        if (empty($id_user)) {
            ...
            $this->session->set_flashdata('message', '<div class="alert alert-success"> Success! User deleted </div>');
            redirect('admin/user_managements');
        }
    }
    ...
    public function process_add($action, $id_user = null) {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $data['created'] = date('Y-m-d H:i:s', time() + 60 * 60 * 5);
            if ($action == 'add') {
